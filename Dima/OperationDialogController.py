...
class OperationDialogController:
    ...
    def save_operation_data(self):
        ...
        if not selected_item:
            show_notification("Выберите товар для операции")
            return
        ...
        # Удаляем выбранный товар из таблицы Current_product для операций "Списать" и "Продать"
    ...
    def insert_operation_data(self, operation_type, client_or_warehouse_id, worker_id, current_time,
        ...
        try:
            ...
            self.cursor.execute(
                "INSERT INTO Operation_product (operation_id, product_id, warehouse_id, quantity) "
                "VALUES (?, ?, ?, ?, ?)",
                (operation_id, product_id, warehouse_id, quantity))  # Здесь передаем operation_type

            # Выполнение коммита через курсор
